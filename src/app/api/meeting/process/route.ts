  ...
  try {
    meetingId = await db.transaction(async (tx) => {
      ...
      for (const tu of body.task_updates ?? []) {
        ...
        await tx.update(pmiTasks).set(patch).where(eq(pmiTasks.id, tu.task_id));

        await tx.insert(taskMeetings).values({ taskId: tu.task_id, meetingId }).onConflictDoNothing();
      ...
      for (const [i, nt] of (body.new_tasks ?? []).entries()) {
        ...
        await tx.insert(taskMeetings).values({ taskId: created.id, meetingId }).onConflictDoNothing();
      ...
      for (const ru of body.risk_updates ?? []) {
        ...
        await tx.update(risks).set(patch).where(eq(risks.id, ru.risk_id));
      ...
      for (const iu of body.initiative_updates ?? []) {
        ...
        await tx.update(valueInitiatives).set(patch).where(eq(valueInitiatives.id, iu.initiative_id));
    ...
    return Response.json(
      { error: "Meeting processing failed", detail: (err as Error).message },
      { status: 500 }
    );
  }
